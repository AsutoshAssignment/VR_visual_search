...
# FIX 1: Load BLIP-2 for query captioning (matches offline indexing in notebook
#         which used Salesforce/blip2-opt-2.7b). For demo inference we use the
#         smaller blip2-opt-2.7b; on CPU-only machines this will be slow but
#         ensures the caption distribution matches the gallery index.
...
@st.cache_resource(show_spinner="Opening gallery embeddings…")
...
# FIX 1: Updated signature to accept Blip2Processor / Blip2ForConditionalGeneration
@torch.no_grad()
...
def itm_rerank(
    ...
    Scoring uses use_itm_head=False (ITC cosine similarity between vision_proj
    and text_proj embeddings). This is intentionally consistent with the
    offline evaluation notebook, which found ITC cosine more robust than the
    binary ITM cross-attention head for cross-view fashion retrieval.
    ...
    # FIX 4: use_itm_head=False is the intentional choice (ITC cosine), consistent
    # with the offline notebook. The binary ITM cross-attention head is less robust
    # for cross-view fashion retrieval as documented in the evaluation notebook.
...
with loading_placeholder.container():
    ...
    status_box.code("Loading CLIP embedding model...", language="bash")
    clip_model, clip_preprocess = load_clip()
    progress.progress(14)

    # -----------------------------------------------------

    status_box.code("Loading FAISS HNSW retrieval index...", language="bash")
    faiss_index = load_faiss_index()
    progress.progress(28)

    # -----------------------------------------------------

    status_box.code("Loading YOLO fashion detector...", language="bash")
    yolo_model = load_yolo()
    progress.progress(42)

    # -----------------------------------------------------

    status_box.code("Loading BLIP captioning model...", language="bash")
    cap_proc, cap_model = load_blip_caption()
    progress.progress(58)

    # -----------------------------------------------------

    status_box.code("Loading BLIP ITM reranker...", language="bash")
    
    progress.progress(74)

    # -----------------------------------------------------

    status_box.code("Loading gallery metadata...", language="bash")
    gallery_df = load_gallery()
    progress.progress(88)

    # -----------------------------------------------------

    status_box.code("Loading gallery embeddings...", language="bash")
...
">
  ...
  <span class="app-subtitle">CLIP · BLIP-2 · HNSW · DeepFashion</span>
</div>
<hr class="divider">
""", unsafe_allow_html=True)
...
with left:
    ...
    if uploaded is not None:
        ...
        if st.session_state["selected_det_idx"] is not None:
            ...
            # FIX 2: top_k_val now drives the FAISS search depth, not just display.
            # Previously TOP_K=15 was hardcoded in retrieve(); slider only trimmed
            # the final list. Now the actual search respects the user's choice.
            top_k_val = st.slider(
                "Results to show",
                min_value=5,
                max_value=TOP_K,
            ...
            if st.button("🔍 Search", type="primary", use_container_width=True):
                ...
                with st.spinner("Embedding & retrieving…"):
                    ...
                    # FIX 3: Use the pre-built HNSW index (faiss_index) for
                    # unfiltered queries (full outfit / direct YOLO detections).
                    # Only fall back to a temporary flat index when category
                    # filtering is needed (upper/lower body), since HNSW does not
                    # support post-hoc subset filtering. This ensures the HNSW
                    # index is actually used at query time as described in the
                    # report, rather than always being bypassed.
                    if selected_label in ("upper body", "lower body"):
                        ...
                        filtered_embs = gallery_embs[valid_indices].astype("float32")
                        faiss.normalize_L2(filtered_embs)

                        # FIX 2 (applied here too): search for exactly top_k_val
                        temp_index = faiss.IndexFlatIP(filtered_embs.shape[1])
                        temp_index.add(filtered_embs)
                        D, I = temp_index.search(q_emb, top_k_val)  # FIX 2

                        retrieved_rows = filtered_gallery.iloc[I[0]].reset_index(drop=True)
                        cand_idx  = np.arange(len(retrieved_rows))
                        dists     = D[0]

                    else:
                        # FIX 3: use the pre-built HNSW index directly
                        filtered_gallery = gallery_df

                        faiss_q = q_emb.copy()
                        faiss.normalize_L2(faiss_q)
                        D, I = faiss_index.search(faiss_q, top_k_val)  # FIX 2 + FIX 3

                        valid_mask    = I[0] >= 0
                        valid_I       = I[0][valid_mask]
                        valid_D       = D[0][valid_mask]

                        retrieved_rows = gallery_df.iloc[valid_I].reset_index(drop=True)
                        cand_idx  = np.arange(len(retrieved_rows))
                        dists     = valid_D
    ...
    if st.session_state["search_done"]:
        ...
        <div style="height:60vh;display:flex;flex-direction:column;
          ...
          <div style="font-size:0.8rem;color:#2A3050;max-width:300px;line-height:1.7">
            ...
            CLIP + BLIP-2 retrieves and re-ranks similar products
          </div>
        </div>
        """, unsafe_allow_html=True)